...
Item {
    ...
    property FileFolder imagesFolder: FileFolder {}
    ...
    LoggingCategory {
        ...
        name: AppFramework.typeOf(exif, true)
    }
    ...
    function propertyValue(fileName, propertyName) {
        ...
        if (debug) {
            console.log(logCategory, "Getting EXIF propertyValue fileName:", fileName, "propertyName:", propertyName);
        }
        ...
        if (!fileInfo.exists) {
            console.warn(logCategory, logCategory, "EXIF file not found:", fileInfo.url);
            return value;
        }
        ...
        if (!exifInfo.load(fileInfo.url)) {
            console.error(logCategory, "Error loading EXIF info:", fileInfo.url);
            exifInfo = undefined;
            gc();
            return value;
        }
        ...
        if (!exifInfo.isExifValid) {
            console.warn(logCategory, "No EXIF data:", fileInfo.url);
            return value;
        }
        ...
        default:
            ...
            if (typeof value === "object") {
                //console.log(logCategory, "EXIF Object value:", value);
        ...
        if (debug) {
            console.log(logCategory, "Got EXIF propertyValue:", propertyName, "=", JSON.stringify(value));
        }
    ...
    function coord(exifInfo, tag, refTag) {
        ...
        if (!Array.isArray(values)) {
            console.error(logCategory, logCategory, "coord tag:", tag, "not an array:", JSON.stringify(values));
            return;
        }
        ...
        // console.log(logCategory, "tag:", tag, "=", JSON.stringify(values), "ref:", refTag, "=", ref, "value:", value);
    ...
    function searchTagSet(tags, tagNameFunction, tagValueFunction, name) {
        //console.log(logCategory, "tags:", JSON.stringify(tags, undefined, 2));
    ...
    Component {
        ...
        ExifInfo {
            Component.onCompleted: {
                if (debug) {
                    console.log(logCategory, "ExifInfo completed");
                }
            }

            Component.onDestruction: {
                if (debug) {
                    console.log(logCategory, "ExifInfo destruction");
                }
            }
        }
    }
