...
QtObject {
    ...
    readonly property string memoryText: hasMemory ? numberToLocaleString(locale, memory) : ""
    ...
    readonly property string kOperationPoint: locale.decimalPoint//"."
    ...
    function setInput(text) {
        result = numberFromLocaleString(locale, text);
        if (!isFinite(result)) {
            result = 0;
        }

        input = numberToLocaleString(locale, result);
    }
    ...
    function doOperation(op) {
        ...
        if (op.length === 1 && ((op >= "0" && op <= "9") || op === kOperationPoint) ) {
            ...
            result = numberFromLocaleString(locale, input);
    ...
    function numberFromLocaleString(locale, text) {
        ...
        if (text > "") {
            ...
            if (lastChar === locale.negativeSign && firstChar !== locale.negativeSign) {
                text = locale.negativeSign + text.substr(0, text.length - 1);
            }
        }
