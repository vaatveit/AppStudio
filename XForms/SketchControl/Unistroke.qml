...
QtObject {
    ...
    Component.onCompleted: {
        ...
        console.log(logCategory, "NumUnistrokes:", numUnistrokes);
    }
    ...
    readonly property LoggingCategory logCategory: LoggingCategory {
        ...
        name: AppFramework.typeOf(unistroke, true)
    }
    ...
    function recognize(points) {
        ...
        console.log(logCategory, arguments.callee.name, "threshold:", threshold, "result:", JSON.stringify(result, undefined, 2));

        if (result.Score < threshold) {
            console.info(logCategory, arguments.callee.name, "name:", result.Name, "score too low:", result.Score, "<", threshold);
            return;
        }
        ...
        case "triangle":
        case "triangle2":
            sketch = SketchLib.createTriangleSketch(points);
            break;
    ...
    function drawUnistroke(ctx, index, width, height) {
        ...
        console.log(logCategory, arguments.callee.name, "index:", index, "name:", stroke.Name, width, height);
    ...
    function addGesture2(name, points) {
        addGesture(name, false, points);
        addGesture(name + "2", true, points);
    }
    ...
    function removeGesture(name) {
        for (var i = 0; i < dollar.Unistrokes.length; i++) {
            ...
            if (stroke.Name === name) {
                if (debug) {
                    console.log(logCategory, arguments.callee.name, "unistroke:", i, name);
                }
        ...
        console.error(logCategory, arguments.callee.name, "Unistroke not found:", name);
    }
